...
class BrokerMW():
    ...
    def configure(self, args):
        ...
        try:
            ...
            self.logger.info(f"BrokerMW::configure - connected to Discovery service - {connect_str}")
    ...
    def handle_reply(self):
        ...
        try:
            ...
            if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
                ...
                self.logger.debug ("BrokerMW::handle_reply TYPE_LOOKUP_ALL_PUBS")
                timeout = self.upcall_obj.lookup_all_pubs_response(disc_resp.lookup_resp)
    ...
    def register(self, name, topiclist):
        ...
        try:
            ...
            register_req.role = discovery_pb2.ROLE_BOTH  # we are a publisher and a subsriber
            # It was observed that we cannot directly assign the nested field here.
            # A way around is to use the CopyFrom method as shown
            register_req.info.CopyFrom(reg_info)  # copy contents of inner structure
            register_req.topiclist[:] = topiclist  # this is how repeated entries are added (or use append() or
            # extend ()
    ...
    def lookup_pubs(self, topiclist):
        ''' lookup publishers for all topics from the discovery service '''

        try:
            self.logger.info("BrokerMW::lookup_pubs")

            # Build Discovery Message
            self.logger.debug("BrokerMW::lookup_pubs - build the outer DiscoveryReq message")
            disc_req = discovery_pb2.DiscoveryReq()
            disc_req.msg_type = discovery_pb2.TYPE_LOOKUP_ALL_PUBS
            # It was observed that we cannot directly assign the nested field here.
            # A way around is to use the CopyFrom method as shown
            self.logger.debug("BrokerMW::lookup_pubs - done building DiscoveryReq message")
            ...
            # now send this to our discovery service
            self.logger.debug("BrokerMW::lookup_pubs - send stringified buffer to Discovery service")
            self.req.send(buf2send)  # we use the "send" method of ZMQ that sends the bytes

            # now go to our event loop to receive a response to this request
            self.logger.info("BrokerMW::lookup_pubs - request sent and now wait for reply")
    ...
    def subscribe(self, topics, publishers):
        ...
        self.logger.debug("BrokerMW::subscribe complete")

    def consume(self):
        try:
            self.logger.debug("BrokerMW::consume")
            ...
            self.disseminate(topic_info)
            self.logger.debug("BrokerMW::consume - {}".format(topic_info))

            # covert bytes to string
            # rec_str = str(bytes_rcvd, 'UTF-8')
            # self.logger.debug("BrokerMW::consume - {}".format(rec_str))
        except zmq.ZMQError as z:
            if z.errno == zmq.EAGAIN:
                pass
            else:
                self.logger.exception("BrokerMW::consume - pub rcv error. {}".format(z))
                raise z
        except Exception as e:
            self.logger.exception("BrokerMW::consume - exception consuming data. {}".format(e))
            raise e
        
    def disseminate(self, topic_info):
        try:
            self.logger.debug("BrokerMW::disseminate")

            buf2send = topic_info.SerializeToString()

            # send the info as bytes. See how we are providing an encoding of utf-8
            self.pub.send_multipart([bytes(topic_info.topic, "utf-8"), buf2send])
            # self.pub.send_string(send_str)
